...
def bullish_reversal_rate(pattern_list, nb_days):
    ...
    base_line = []
    if os.path.getsize("last_base_line_concat.json") > 1024:
        with open("last_base_line_concat.json") as base_line_file:
            base_line = json.load(base_line_file)
    pattern_line = dict()
    for pattern in pattern_list:
        pattern_line[pattern] = []
    if os.path.getsize("last_pattern_line_concat.json") > 1024:
        with open("last_pattern_line_concat.json", "r") as dic_file:
            pattern_line = json.load(dic_file)
    it_file = open("iteration_number.txt", "r")
    last_treated_index = int(it_file.readline())
    it_file.close()
    with zipfile.ZipFile(zip_path, "r") as dir_path:
        for index, file_name in tqdm(enumerate(dir_path.namelist())):
            if index > last_treated_index:
                with dir_path.open(file_name) as file:
                    ...
                    if not data_frame.empty:
                        try:
                            ...
                            with open("iteration_number.txt", "w") as it_file:
                                it_file.write(f"{index}")
